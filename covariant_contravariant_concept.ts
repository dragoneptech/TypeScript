...
// 协变 (Covariance) 与逆变 (Contravariance) 是 TypeScript 类型系统中的重要概念
// 理解它们有助于编写类型安全的代码
...
// 概念：
// 协变 (Covariance) 允许子类型向父类型转换
// 逆变 (Contravariance) 允许父类型向子类型转换
// 不变 (Invariance) 则不允许任何方向转换
...
// 启用严格函数类型
// TypeScript 默认对函数参数进行逆变检查
// 启用 strictFunctionTypes 会强制执行此规则
...
// // 正确的赋值
// const getAnimalName: GetName = (animal) => animal.name;
// let getDogBreed: GetDogBreed = (dog) => dog.breed;
// getDogBreed = getAnimalName; // 输入参数逆变，正确！因为 GetName 的参数类型更宽泛，可以接受 Dog 类型

// console.log("动物名称: " + getDogBreed({ name: "旺财", breed: "哈士奇" }));
...
// Producer<Dog> 可以赋值给 Producer<Animal>（协变），因为后续调用协变方法 produce
// const animalProducer: Producer<Animal> = new DogProducer();

// Consumer<Animal> 可以赋值给 Consumer<Dog>（逆变），因为后续调用逆变方法 consume
...
// 数组协变：注意可变性带来的潜在问题
// 对象属性协变：子类型属性可以赋值给父类型属性
// 根据类方法的协变与逆变特性设计泛型接口，确保类型安全
