...
// 原始导入路径
// 使用别名导入
// tsconfig.json 中的配置 paths 会将别名映射到实际的文件路径，从而在编译时正确解析模块
...
// tsc --init 生成 tsconfig.json 文件后，添加以下配置：
...
// 最佳实践：使用路径别名 (Path Aliases) 可以让代码更简洁，但不要滥用，建议统一命名规范。
...
// baseUrl：设置基准路径
// 通配符：使用 * 匹配任意字符
// 构建工具：Webpack/Vite 需要与 tsconfig 同步配置

// 建议：为常用目录设置路径别名，提高代码可读性和可维护性

// 在最新版的 TypeScript 中，bseUrl 已经不再是必须的配置项
// 但在使用 paths 时，仍然建议设置 baseUrl，以确保路径解析的一致性和可预测性