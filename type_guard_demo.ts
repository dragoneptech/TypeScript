...
// 概念说明：类型守卫的核心原理是"类型缩窄"（Type Narrowing）
// 通过条件判断，TypeScript 会自动将联合类型缩小为具体的类型


// 类型守卫 - 类型缩窄流程
// 原始联合类型
// typeof、 instanceof、in 操作符进行条件判断
// 类型确定后的操作

// 类型守卫方式
// typeof 用于基本类型/原始类型的检查 (primitive types)
// instanceof 用于类实例的检查 (class instances)
// in 用于对象属性的检查 (object properties)
// 自定义类型守卫: is 类型谓词 (custom type predicates), 例如: x is String
// 自定义类型守卫函数可以通过返回布尔值来判断变量是否属于某个特定类型
...
// 注意：typeof 对于数组和 null 都会返回 "object"
// 如果需要精确区分数组和对象，需要使用其他方式
...
// 可辨识联合与类型守卫 (Discriminated Unions and Type Guards)
...
// 说明：可辨识联合是 TypeScript 中最推荐使用的模式之一
// 它通过一个公共的字面量属性（通常是 kind 或 type）来区分不同的类型成员，使代码既类型安全又易于维护

// null 和 undefined 检查
// 在 TypeScript 中，null 和 undefined 是两个特殊的值，它们有自己的类型
// null 的类型是 null，undefined 的类型是 undefined

// 定义一个可能为 null 或 undefined 的变量
// let nullableValue: string | null | undefined;

// 在 TypeScript 中，不能初始化赋值 null，否则会报错

// 处理可能为 null 或 undefined 的值时，直接的相等性检查也是有效的类型守卫
// if (nullableValue === null) {
//     console.log("值为 null");
// } else if (nullableValue === undefined) {
//     console.log("值为 undefined");
// } else {
//     // 此时 TypeScript 知道 nullableValue 不是 null 或 undefined
//     // 因此可以安全地访问字符串的属性
//     console.log("字符串长度: " + nullableValue!.length);
// }


// 使用 ! 操作符强制断言非空
// let nonNullableValue: string | null = "hello";
// console.log("字符串长度: " + nonNullableValue!.length);
...
// 提示：在启用 strictNullChecks 后，建议始终进行 null 检查
// 可以使用可选链（?.）和空值合并（??）来简化代码
...
// 以下在 TypeScript 中可能报错，但是在 JavaScript 中是合法的
// 短路运算符 && 的行为如下：
// console.log(false && "Hello");  // 输出 false
// console.log(0 && "Hello");      // 输出 0
// console.log(-0 && "Hello");     // 输出 -0
// console.log("" && "Hello");    // 输出 ""
// console.log(null && "Hello");  // 输出 null
// console.log(undefined && "Hello");  // 输出 undefined
...
// 建议：在处理联合类型时，优先考虑使用可辨识联合模式
// 它不仅代码更清晰，还能充分利用 TypeScript 的类型推断能力
