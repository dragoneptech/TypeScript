...
// 字面量类型
// let direction: "up" | "down" | "left" | "right"; // 只能赋这四个值之一
...
// 为什么需要字面量类型?
// 1. 提高类型安全性: 限制变量只能取特定的值，避免无效的赋值
// 2. 提供更好的代码提示: 编辑器可以根据字面量类型提供更准确的自动补全和提示
// 3. 增强可读性: 通过明确的类型定义，代码的意图更加清晰

// 在 TypeScript 中，默认的类型如 string 和 number 过于宽泛
// 有时我们需要更精确的类型控制。例如，一个变量只能取特定的几个值
// 字面量类型正是为了解决这类问题而生的

// 概念说明：字面量类型是泛型类型（string、number）的具体值
// 例如 "up" 是 string 的子类型，只能赋值给它的值是特定的字符串

// 字符串字面量类型
// 字符串字面量类型将变量的取值限制为特定的字符串
// 这在需要限制变量只能取某些固定值时非常有用

// 定义字符串字面量类型 (String Literal Type)
...
// 使用 Type Alias & Number Literal Types 数字字面量模拟枚举
// 定义一周的天数
...
// 实际上 boolean 就是 true | false 的联合类型 (Type Alias)
// 所以这两种写法是等价的
// const flag: boolean = true;
// const flag: true | false = false;

// console.log("是否激活: " + isActive);

// 提示：虽然 boolean 本身已经足够使用
// 但在需要明确区分 true 和 false 的类型时，可以使用 true 或 false 作为布尔字面量类型


// 对象字面量类型 （Object Literal Types）
// 对象字面量类型可以定义对象的结构，并可以使用 readonly 修饰符将属性设为只读
// 这在需要创建不可变对象时非常有用
// 对象字面量类型与类型别名结合使用，可以创建复杂的类型结构
...
// 使用 var 声明并赋值
// TypeScript 会推断为宽泛类型
// 更推荐使用 let 或 const 来声明变量，以获得更精确的类型推断
// let uname = "Alice";    // 类型: string（因为 let 可以重新赋值）
// let age = 25;          // 类型: number
// let enabled = true;    // 类型: boolean

// console.log("名字: " + uname + ", 年龄: " + age + ", 启用: " + enabled);
...
// 模板字面量类型 (Template Literal Type)
// 模板字面量类型使用 JavaScript 模板字符串 (Template Strings) 的语法来创建新的类型
// 它允许我们动态地生成字符串类型
...
// 使用模板字面量类型
// let eventVar: EventName = "onClick";
// let handler: Handler = "handleSubmit";

// console.log("事件: " + eventVar);
...
// 模板字面量类型：可以与内置工具类型结合使用，创建更复杂的类型
...
// 对象字面量：与类型别名结合使用，定义对象结构，可使用 readonly
