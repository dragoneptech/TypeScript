...
// infer 是 TypeScript 条件类型 (Conditional Types) 中的关键字，用于从类型中推断出新的类型
...
// const value: ValueOf<Promise<string>> = "Hello, TypeScript!"; // V 被推断为 string
...
// 概念说明：infer 是 "infer" 的缩写，意为"推断"
// 它只能在条件类型的 extends 子句中使用，用于声明一个待推断的类型变量
...
// async function getValue(promise: Promise<string>) {
//     const value: ValueOf<typeof promise> = await promise;
//     console.log("提取的值类型: " + value);
//     return value;
// }
// const value: ValueOf<typeof promise> = await getValue(promise); // value 的类型被推断为 string
// // const value: ValueOf<typeof promise> = "world";
...
// const promiseResult: R2 = fetchUser(1);
// console.log("返回类型: " + JSON.stringify(result));
// console.log("Promise 返回类型: " + promiseResult);
...
// console.log("参数类型: " + typeof nameParam);
...
// const flatValue: Flat = "Flattened Value";
