...
// 相对路径：使用相对路径导入本地模块（./、../）
// 绝对路径需要在 tsconfig.json 中配置 baseUrl 和 paths，并配合 @ 符号使用

// 模块扩展名：TypeScript 编译时会自动处理扩展名(?)
// CommonJS 模块可以省略扩展名，ESM 模块需要明确指定扩展名（如 .js、.ts、.tsx 等）
// 但是在 TypeScript 中，推荐使用 ESM 规范，并明确指定文件扩展名
// 推荐在 tsconfig.json 中的 moduleResolution 设置为 "node"，是目前最常用的方式
// 此时 node.js 配合 package.json 中的 "type" 字段为 "commonjs"，可以省略扩展名
// node.js 配合 package.json 中的 "type" 字段为 "module"，需要明确指定扩展名
