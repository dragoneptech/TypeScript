...
// 错误处理 (Error Handling) 是保证程序健壮性的重要环节
// TypeScript 提供了强大的类型系统，可以帮助开发者更好地处理和预防错误

// TypeScript 中常见的错误处理模式和最佳实践

// 错误处理方式
// try-catch: 捕获并处理错误或异常
// Result 类型：返回结果而非抛错误或异常
// 自定义错误：结构化错误信息
...
// 概念说明：错误处理有两种主要方式：
// 异常处理（try-catch）和返回值处理（Result 类型）
// 前者使用抛出异常表示错误，后者使用返回值携带错误信息
...
// function safeDivide(a: number, b: number): Result<number> {  // Result 使用默认值 Error
//     // 检查除数是否为零
//     if (b === 0) {
//         // 返回错误结果
...
//     // 处理结果
//     // 可辨识联合类型 (Discriminated Union), TypeScript 可以根据 ok 属性的值来推断类型
//     if (result.ok) {  
