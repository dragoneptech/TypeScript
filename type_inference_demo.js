...
// 概念说明：类型推断是 TypeScript 编译器分析代码上下文，自动确定变量类型的过程
// 它基于初始值、函数返回值、参数类型等多个维度进行推断
// 基础类型推断
...
// 说明：TypeScript 使用 `var` 声明变量时，会根据初始值推断类型
// 如果初始值是数字，类型就是 number；如果是字符串，类型就是 string
// 函数返回类型推断
...
// 上下文推断：当变量在特定上下文中使用时（如回调函数、事件处理等），
// TypeScript 会根据上下文自动推断类型。这种推断叫做"上下文类型推断"
// 最佳通用类型推断
...
// 联合类型：当数组包含多种类型时，TypeScript 会推断为联合类型
// 例如 (number | string)[] 表示数组元素可以是 number 或 string
// 类型推断的限制
...
// var unknown;
// console.log("未指定类型: " + typeof unknown); // 输出: undefined
// // 可以赋任何类型的值
// unknown = "hello";
// console.log("未指定类型: " + typeof unknown); // 输出: string
// unknown = 123;
// console.log("未指定类型: " + typeof unknown); // 输出: number
// // 这种行为容易导致类型错误
...
// 警告：避免声明变量时不赋值
// 最好在声明时提供初始值，或者显式声明类型，以充分利用 TypeScript 的类型检查功能
// 泛型函数推断
