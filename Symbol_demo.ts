...
// // const obj: any = {};
// const obj: { [key: symbol]: string; [key: string]: string } = {};
...
// 注意事项
// 唯一性：每次 Symbol() 调用的值都不同, 即使描述相同也不相等
// 不可枚举：Symbol 属性不会出现在 for...in 循环中，也不会被 Object.keys()、Object.getOwnPropertyNames() 返回
...
// 总结
// Symbol 是 TypeScript 中重要的原始类型 (primitive type)，用于创建唯一标识符，确保对象属性的唯一性
// 主要特点：

// 唯一性：每次创建的 Symbol 都不相等, 即使描述相同
// 属性键：可用作对象的唯一属性键
