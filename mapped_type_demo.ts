...
// 使用 selfPartial 类型
// type PartialUser = selfPartial<User>;
...
// 语法解释：[P in keyof T] 表示遍历 T 类型的所有键
// ? 修饰符将属性设为可选
...
// 测试只读类型
// const readonlyUser: selfReadonly<User> = { name: "Alice", age: 25 };
// readonlyUser.age = 30; // 错误：只读属性不能修改

// 测试可选类型
// const optionalUser: Optional<User> = { name: "Bob" };
...
// 键名映射（Key Remapping）
// 映射类型 (Mapped Types) 还可以使用 as 关键字来重映射键名
// 这在需要统一修改键名格式时非常有用
...
// 模板字面量类型（Template Literal Types）：使用 `${Prefix}${Capitalize}` 可以动态生成新的键名
// 内置工具类型 Capitalize
// 交叉类型 &
...
// 实现 Omit：排除指定键
// 使用条件类型过滤键
// type selfOmit<T, K extends keyof T> = {
//     // P 遍历 T 的所有键
...
// type UserWithoutPassword = selfOmit<User, "password">;
